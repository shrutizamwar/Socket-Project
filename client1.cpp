...
int main(){
	...
	clientmap cmap = getMap("client1.txt");	
	...
	cout<<"\n The Client 1 has received a request with search word " << search <<", which maps to key "<<key<<endl;
	...
	cout<<"\n The Client1 sends the request "<< message<< " to the server1 with port number "<<ntohs(server_addr.sin_port);
	...
	cout<<"\n The Client1's port number is "<<ntohs(my_addr.sin_port);
	...
	cout<<"\n The Client 1 received the value "<<message<<" from Server 1 with port number " <<ntohs(server_addr.sin_port);
	...
	/** Close the UDP connection with server1*/
	close(sockfd);
	return 0;
}
